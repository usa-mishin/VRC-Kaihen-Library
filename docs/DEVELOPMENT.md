...
- BLMの未読 `latestDownloadableAvailable` 通知が持つ最新ZIP名を、ローカルの展開フォルダー名・UnityPackage名の系列と版番号に照合し、旧版の可能性が高いUnityPackageへ「新バージョンあり」を表示する。複数パッケージ商品で系列を特定できない場合は誤表示を避ける。
